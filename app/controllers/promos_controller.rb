class PromosController < ApplicationController
  ...
  def index
    ...
    @deals = @promos
    @promos_next = Promo.find_next_items(merchant_id, params[:classification]).asc(:countdown_period_start)
    @promos_next_count = @promos_next.count
    @promos_coming = @promos_next.first
    @dateage = Time.now.strftime('%Y %B %d %H %M')
    @tryme = Time.now.in_time_zone("Hong Kong")
    @try = @tryme.strftime('%Y %B %d %H %M')
  end
  ...
  def show
    ...
    @title = @promo.name
    @users = @promo.users
    @promo_branches_count = @promo.branches.count
    @time = Time.now.in_time_zone("Hong Kong")
  end
  ...
  def override_period_start(promo)
    promo._min_period_start = Time.now.in_time_zone("Hong Kong")
