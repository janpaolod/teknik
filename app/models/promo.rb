...
class Promo
  ...
  references_and_referenced_in_many :users, :inverse_of => :promos
  references_and_referenced_in_many :classifications,  :inverse_of => :promos
  references_and_referenced_in_many :branches,  :inverse_of => :promos
  ...
  scope :not_expired, where(:countdown_period_start.lte => Time.now,
                            ...
                            :status => :active)
  scope :next_deal, where(:status => :active)
  ...
  def countdown_for_next_deal
    self.countdown_period_start.to_i - DateTime.now.to_i
  end
  ...
  def self.find_next_items( merchant_id, permalink)
    promos = merchant_id ? self.where(:merchant_id => merchant_id) : all
    if permalink.nil?
      promos.next_deal.where(:countdown_period_start => {'$gt' => DateTime.now, '$lt' => 7.days.from_now}) 
    else
      c = Classification.where(:permalink => permalink).first
      c.promos.next_deal.where(:countdown_period_start => {'$gt' => DateTime.now, '$lt' => 7.days.from_now}) 
    end
  end
...
public
  ...
  def period_start_new_format
    self.countdown_period_start.strftime("%b %d, %Y %I:00 %p")
  end
  ...
  def period_end_new_format
    self.countdown_period_end.strftime("%b %d, %Y %I:00 %p")
  end
  
  def validity_start_new_format
    self.coupon_validity_start.strftime("%b %d, %Y")
  end
  
  def validity_end_new_format
    self.coupon_validity_end.strftime("%b %d, %Y")
  end
  # Deprecated APIs
